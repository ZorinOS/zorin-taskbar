...
var PreviewMenu = Utils.defineClass({
    ...
    _refreshGlobals: function() {
        ...
        animationTime = WINDOW_PREVIEW_ANIMATION_TIME * .001;
        aspectRatio.x = {
            size: WINDOW_PREVIEW_ASPECT_RATIO_X,
            fixed: WINDOW_PREVIEW_FIXED_X
        };
        aspectRatio.y = {
            size: WINDOW_PREVIEW_ASPECT_RATIO_Y,
            fixed: WINDOW_PREVIEW_FIXED_Y
        };
    ...
    _updateClip: function() {
        ...
        if (geom.position == St.Side.LEFT) {
            x = this.panel.monitor.x + this.panel.dtpSize + panelBoxTheme.get_padding(St.Side.LEFT) + MARGIN_SIZE;
        } else if (geom.position == St.Side.RIGHT) {
            x = this.panel.monitor.x + this.panel.monitor.width - (this.panel.dtpSize + previewSize) - panelBoxTheme.get_padding(St.Side.RIGHT) - MARGIN_SIZE;
        } else if (geom.position == St.Side.TOP) {
            y = this.panel.monitor.y + this.panel.dtpSize + panelBoxTheme.get_padding(St.Side.TOP) + MARGIN_SIZE;
        } else { //St.Side.BOTTOM
            y = this.panel.monitor.y + this.panel.monitor.height - (this.panel.dtpSize + panelBoxTheme.get_padding(St.Side.BOTTOM) + previewSize + headerHeight) - MARGIN_SIZE;
        }
    ...
    _getThemeBackground: function(reload) {
        if (reload || !this._themeBackground) {
            ...
            let fakeTheme = fakeTooltip.get_theme_node();
            this._themeBackground = fakeTheme.get_background_color();
            Main.uiGroup.remove_child(fakeTooltip);
        }
...
var Preview = Utils.defineClass({
    ...
    _init: function(previewMenu) {
        ...
        this._padding = WINDOW_PREVIEW_PADDING * scaleFactor;
        ...
        if (headerHeight) {
            ...
            headerBox.set_size(headerHeight, headerHeight);
    ...
    adjustOnStage: function() {
        ...
        if (!headerHeight) {
            ...
            if (isTopHeader) {
                closeButtonBorderRadius += (isLeftButtons ? '0 0 8px 0;' : '0 0 0 8px;');
            } else {
                closeButtonBorderRadius += (isLeftButtons ? '0 8px 0 0;' : '8px 0 0 0;');
            }
        }
