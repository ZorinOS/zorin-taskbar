 ...
 *
 * Credits:
 * This file is based on code from the Dash to Panel extensionq
 */
...
export const PreviewMenu = GObject.registerClass(
  ...
  class PreviewMenu extends St.Widget {
    ...
    open(appIcon, preventCloseWindow) {
      if (this.currentAppIcon != appIcon) {
        ...
        if (!this.opened) {
          ...
          setStyle(
            this.menu,
            'padding: 0; margin: 0; border: none; background-image: none;',
          )
        }
    ...
    _peek(window) {
      ...
      let focusWindow = () =>
        this._focusMetaWindow(PEEK_MODE_OPACITY, window)
  ...
  class Preview extends St.Widget {
    ...
    _updateHeader() {
      if (headerHeight) {
        ...
        setStyle(
          this._windowTitle,
          'max-width: 0px; padding-left: 8px; padding-right: 8px; text-align: center;',
        )
        this._updateWindowTitle()
      }
    }
    ...
    _getRgbaColor(offset, alpha) {
      ...
      return Utils.getrgbaColor(
        this._previewMenu._getThemeBackground(),
        alpha,
        offset,
      )
    }
