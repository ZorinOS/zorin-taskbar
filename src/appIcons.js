...
export const DEFAULT_PADDING_SIZE = 0
  ...
  class TaskbarAppIcon extends AppDisplay.AppIcon {
        ...
        switch (type) {
          ...
          case DOT_STYLE.CILIORA:
            ...
            preDraw = () => {
              ...
              cr.arc.apply(
                cr,
                (isHorizontalDots ? [radius, radius] : [radius, radius]).concat([radius, 0, 2 * Math.PI])
              )
              cr.arc.apply(
                cr,
                (isHorizontalDots ? [length - radius, radius] : [radius, length - radius]).concat([radius, 0, 2 * Math.PI])
              )
              cr.rectangle.apply(
                cr,
                (isHorizontalDots ? [radius, 0, length - size, size] : [0, radius, size, length - size])
              )
            }
            draw = (i) => {
              ...
              cr.arc.apply(
                cr,
                (isHorizontalDots ? [dist, radius] : [radius, dist]).concat([radius, 0, 2 * Math.PI]),
              )
            }
            break
          case DOT_STYLE.DOTS: {
    ...
    _maybeUpdateNumberOverlay() {
      ...
      if (showNotifications) {
        this._numberOverlayLabel.set_x_align(Clutter.ActorAlign.END)
        this._numberOverlayLabel.set_y_align(Clutter.ActorAlign.START)
        this._numberOverlayLabel.set_x_expand(true)
        this._numberOverlayLabel.set_y_expand(true)
      } else {
        this._numberOverlayLabel.set_x_align(Clutter.ActorAlign.START)
        this._numberOverlayLabel.set_y_align(Clutter.ActorAlign.START)
        this._numberOverlayLabel.set_x_expand(false)
        this._numberOverlayLabel.set_y_expand(false)
      }
    ...
    _updateNumberOverlay() {
      ...
      let fontSize = Math.round(
        Math.max(minFontSize, 0.2 * panelSize) / Utils.getScaleFactor(),
      )
